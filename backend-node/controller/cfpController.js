...
//         const ECuser = new ECmodel({
//             username: savedCFP.ECname,
//             email: savedCFP.ECemail,
//             password: hash,
//             Oname: savedCFP.conferenceName,
//             CFPid: savedCFP._id
//         })
...
exports.createCFP = async (req, res) => {
    try {
        ...
        await EmailController.sendEmail({ to, subject, text });
        // saving EC
        const savedEC = await ECController.createEC({ username, email, password, Oname, CFPid })
        res.status(200).json({ message: 'EC created successfully', ec: savedEC , CFP: savedCFP});
    } catch (err) {
        res.status(400).json({ message: err.message });
    }
};
...
exports.updateCFPById = async (req, res) => {
    try {
        const updatedCFP = await CFP.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (updatedCFP === null) {
            return res.status(404).json({ message: "CFP not found" });
        }
        res.json(updatedCFP);
